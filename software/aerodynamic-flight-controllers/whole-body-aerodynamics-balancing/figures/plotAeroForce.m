...
%% Define settings
varName       = 'aerodynamic_forces_SCOPE';
yVarLabel     = '$F_a$ [N]';
...
y1 = squeeze(sum(out.(varName).signals.values(1,:,:),2));
y2 = squeeze(sum(out.(varName).signals.values(2,:,:),2));
y3 = squeeze(sum(out.(varName).signals.values(3,:,:),2));
