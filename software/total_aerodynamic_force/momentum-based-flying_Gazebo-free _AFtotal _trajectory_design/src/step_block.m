...
%                 obj.contacts.compute_contact(obj.robot, torque, generalized_total_wrench, obj.state.base_pose_dot, obj.state.s_dot);
        ...
        function [generalized_aerodynamics_wb,aerodynamics_forces_wb,relative_velocity_wb,AoA_wb,beta,Fa_drag,Fa_normal]=compute_aero_wholebody(obj)
            ...
            Fa_normal=zeros(3,1);
                ...
                relative_velocity_wb=obj.aerodynamics.get_com_va(obj.robot,obj.state.base_pose_dot,obj.state.s_dot);% com Va
                
%                 ini_relative_velocity=obj.aerodynamics.compute_relative_v(obj.robot,zeros(6,1),zeros(23,1),obj.link_frame(i));
               [w_kaxis_chest,w_iaxis_chest]=obj.aerodynamics.chest_rot(obj.robot);
                AoA_wb=obj.aerodynamics.compute_AoA(w_kaxis_chest,relative_velocity_wb);
                beta=obj.aerodynamics.compute_beta(w_kaxis_chest,w_iaxis_chest,relative_velocity_wb);
                [Fa_total,Fa_drag,Fa_side,Fa_normal]=obj.aerodynamics.compute_total_af(w_kaxis_chest,w_iaxis_chest,relative_velocity_wb);

                aerodynamics_forces_wb=Fa_total;
               
                J_com=obj.robot.get_com_jacobian(); % 3x29
                generalized_aerodynamics_wb=J_com'*aerodynamics_forces_wb;% 29x1
