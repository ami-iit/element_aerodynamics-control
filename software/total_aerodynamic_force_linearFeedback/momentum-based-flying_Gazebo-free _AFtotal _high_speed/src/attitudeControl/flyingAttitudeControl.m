function [H_angMomentum, g_angMomentum, ATilde_angular, BTilde_angular, deltaTilde_angular, verifyAngMomAndInertia, w_L_angMom_des] = ...
    ...
    if isempty(b_I_c_0) % the data only fills the b_I_c_0 at the beginning which means t=0
    ...
    b_LDot_angMom       = b_R_w * LDot_estimated(4:6) - wbc.skew(b_omega_b) * b_L_angMom;%LDot_estimated(4:6) is in world frame
    ...
    % attitude control in BODY coordinates Rotational PID controll
...
%     coordinates into the angular momentum in body coordinates:
    ...
    w_L_angMom_des         =  w_R_b_des * b_L_angMom_des; %desired angular momentum in world frame
end 