classdef step_block < matlab.System & matlab.system.mixin.Propagates
    ...
    methods (Access = protected)
        ...
        function [w_H_b, s, base_pose_dot, s_dot, jet_intensities, wrench_left_foot, wrench_right_foot,aerodynamics_forces_single,relative_velocity_single,AoA_single,w_kaxis_link_single] = stepImpl(obj, jets_input, torque)
            ...
            [generalized_aerodynamics_wb,aerodynamics_forces_single,relative_velocity_single,AoA_single,w_kaxis_link_single]=obj.compute_aero_wholebody();
            ...
            % for dropping test
            jet_intensities=(1e-5)*zeros(4,1);
            generalized_jet_wrench=(1e-5)*zeros(29,1); 
...
%                 obj.contacts.compute_contact(obj.robot, torque, generalized_total_wrench, obj.state.base_pose_dot, obj.state.s_dot);
        ...
        function [generalized_aerodynamics_wb,aerodynamics_forces_single,relative_velocity_single,AoA_single,w_kaxis_link_single]=compute_aero_wholebody(obj)
            ...
            generalized_aerodynamics_wb=zeros(obj.robot.NDOF+6,1);
                ...
                relative_velocity_single=obj.aerodynamics.compute_relative_v(obj.robot,obj.state.base_pose_dot,obj.state.s_dot,obj.af_frame);
                
%                 ini_relative_velocity=obj.aerodynamics.compute_relative_v(obj.robot,zeros(6,1),zeros(23,1),obj.link_frame(i));
                w_kaxis_link_single=obj.aerodynamics.compute_kaxis(obj.robot,obj.af_frame);
                
                AoA_single=obj.aerodynamics.compute_AoA(w_kaxis_link_single,relative_velocity_single);
                

%                 aerodynamics_forces_single=obj.aerodynamics.compute_af_link(ini_relative_velocity,w_kaxis_link,obj.link_frame(i)); 
                aerodynamics_forces_single=obj.aerodynamics.compute_af_link(relative_velocity_single,w_kaxis_link_single,obj.af_frame);
                
                aerodynamics_wrench_single=obj.aerodynamics.compute_gener_af(obj.robot,aerodynamics_forces_single,obj.af_frame);
        ...
        function [out, out2, out3, out4, out5, out6, out7,out8,out9,out10,out11] = getOutputSizeImpl(~)
        ...
        function [out, out2, out3, out4, out5, out6, out7,out8,out9,out10,out11] = getOutputDataTypeImpl(~)
        ...
        function [out, out2, out3, out4, out5, out6, out7,out8,out9,out10,out11] = isOutputComplexImpl(~)
        ...
        function [out, out2, out3, out4, out5, out6, out7,out8,out9,out10,out11] = isOutputFixedSizeImpl(~)
