function [c_collision, grad_c_collision] = computeSelfCollisionsAndGradient(KinDynModel, Config)
    ...
    for i = 1:length(Config.collisions.framesList)
        ...
        for j = 1:length(Config.collisions.framesList)
            ...
            if (i > j) && ~((i == 12 && j == 3) || (i == 12 && j == 4))
                ...
                [c_coll_ij, grad_coll_ij, isColliding] = checkCollisionBetweenTwoLinks(w_H_i, w_H_j, centers_i, centers_j, b_centers_i, b_centers_j, radiuses_i, radiuses_j, J_i, J_j);
