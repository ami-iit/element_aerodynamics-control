...
function simulinkStaticGUI_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<INUSL>
    ...
    % Import the configuration list
    names = evalin('base', 'ConfigGUI.joints_references_names');
    set(handles.configListbox,'string',names);
...
function switchButton_Callback(hObject, eventdata, handles) %#ok<INUSL>
    ...
    handles.output = hObject;
    currentState   = get(handles.configListbox,'value') - 1;

    % Assign current state to the state machine
    set_param('WindTunnelControl/WIND TUNNEL CONTROLLER/WIND TUNNEL CONTROL/InputGUI','Value',num2str(currentState))
