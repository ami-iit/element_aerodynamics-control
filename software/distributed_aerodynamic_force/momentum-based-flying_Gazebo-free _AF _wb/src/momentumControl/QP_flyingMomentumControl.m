...
function QP_flyingMomentumControl(block)
    ...
    function setup(block)
        ...
        block.OutputPort(1).Dimensions = 4;                       % jetIntensitiesDot_star
        block.OutputPort(2).Dimensions = 12;                      % contactForcesDot_star
        block.OutputPort(3).Dimensions = block.DialogPrm(1).Data; % jointVel_star
        block.OutputPort(4).Dimensions = 1;                       % exitFlag       
    ...
    function Outputs(block)    
        ...
        lowerBoundQP          = block.InputPort(3).Data; 
        upperBoundQP          = block.InputPort(4).Data;
        feetContactIsActive   = block.InputPort(5).Data;
    
        % pre-processing: switch from balancing to flying
        if feetContactIsActive
            ...
            % remove the contact forces from the optimization procedure
            H              = HessianMatrixQP([1:4, 17:end], [1:4, 17:end]);
            g              = gVectorQP([1:4, 17:end]);
            lb             = lowerBoundQP([1:4, 17:end]);
            ub             = upperBoundQP([1:4, 17:end]);            
        end
            
        % QPoases optimization
        [u, ~, exitFlagQP, ~, ~, ~] = qpOASES(H, g, lb, ub);    
    
        % post-processing: switch from balancing to flying
        if feetContactIsActive
            
            block.OutputPort(1).Data   = u(1:4);      % jetIntensitiesDot_star
            block.OutputPort(2).Data   = u(5:16);     % contactForcesDot_star
            block.OutputPort(3).Data   = u(17:end);   % jointVel_star
            block.OutputPort(4).Data   = exitFlagQP;  % exitFlagQP
        else
            block.OutputPort(1).Data   = u(1:4);      % jetIntensitiesDot_star
            block.OutputPort(2).Data   = zeros(12,1); % contactForcesDot_star
            block.OutputPort(3).Data   = u(5:end);    % jointVel_star
            block.OutputPort(4).Data   = exitFlagQP;  % exitFlagQP
        end        
    end
